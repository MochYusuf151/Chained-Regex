...
function unescapeReplacement(str){
  // Allows users to type \n \t \\ etc in the substitute field and have them treated as real chars.
  try{
    const safe = str.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    // undo double-escaping of the actual intended escape sequences
...
function runScriptStage(input, code){
  ...
  state.stages.forEach((stage, idx) => {
    ...
    try{
      if (source == null || source === ''){
        output = '';
      } else if (stage.processingMode === 'regex'){
        if (!stage.regex.search){ output = source; }
        else output = runRegexStage(source, stage.regex);
      } else {
        output = runScriptStage(source, stage.script);
      }
    }catch(err){
      error = err.message;
      output = '';
...
function renderProcessingArea(wrap, stage){
  ...
  if (stage.processingMode === 'regex'){
    ...
    area.innerHTML = `
      <div class="regex-grid">
        <div>
          ...
          <input type="text" class="input mono" data-role="searchField" placeholder="e.g. (\\w+)@(\\w+)" />
        ...
        <div>
          ...
          <input type="text" class="input mono" data-role="substituteField" placeholder="e.g. $1 [at] $2" />
    ...
    `;
    area.querySelector('[data-role="searchField"]').value = stage.regex.search;
    area.querySelector('[data-role="substituteField"]').value = stage.regex.substitute;
  } else {
    area.innerHTML = `
      <textarea class="textarea mono" data-role="scriptField" rows="4"></textarea>
      <div class="script-hint">function(input) { ${escapeHtml('/* your code */')} }<br>Receives <b>input</b> as a string, must <b>return</b> a string. Runs in your browser only.</div>
    `;
    area.querySelector('[data-role="scriptField"]').value = stage.script;
  }
}
...
function updateCounts(wrap, stage){
  ...
  const source = stage.inputMode === 'table' ? serializeDelimited(stage.table.rows, stage.table.delimiter) : stage.inputString;
...
function bindInputAreaEvents(wrap, stage){
  ...
  function importText(text){
    ...
    if (stage.table.delimiter === 'auto') { /* keep auto so future pastes redetect */ }
    renderTable(wrap, stage);
    bindTableCellEvents();
    refreshAllOutputs();
  }
  ...
  function bindTableCellEvents(){
    ...
    tw.querySelectorAll('.cell').forEach(cell => {
      ...
      cell.addEventListener('paste', (e) => {
        // allow pasting a CSV block starting from a specific cell without nuking whole table
        const text = (e.clipboardData || window.clipboardData).getData('text');
...
function bindProcessingAreaEvents(wrap, stage){
  if (stage.processingMode === 'regex'){
    ...
    subField.addEventListener('input', (e) => { stage.regex.substitute = e.target.value; refreshAllOutputs(); });
    iterField.addEventListener('input', (e) => { stage.regex.iteration = Math.max(1, parseInt(e.target.value,10) || 1); refreshAllOutputs(); });
    ...
    wrap.querySelector('[data-role="replaceModeSeg"]').addEventListener('click', (e) => {
      const btn = e.target.closest('button'); if (!btn) return;
